// ═══════════════════════════════════════
// AIdark — Device Fingerprint v2
// src/lib/fingerprint.ts
// FIXES v2:
//   [1] giveBonusMessages: removed unused `count` parameter
//       (the actual limit is FREE_LIMIT which is checked elsewhere)
// ═══════════════════════════════════════
...
// FIX v2 [1]: removed unused `count` parameter
export function giveBonusMessages(): void {
  if (wasBonusGiven()) return;

  localStorage.setItem(BONUS_GIVEN_KEY, 'true');
  localStorage.setItem(BONUS_USED_KEY, '0');
  localStorage.setItem(BONUS_TS_KEY, String(Date.now()));
}
